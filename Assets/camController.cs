...
public class camController : MonoBehaviour {
	...
	void Update () {
        ...
        if (player.transform.GetChild(0).GetComponent<spacecraft>().rotating_planet != null
            && player.transform.GetChild(0).GetComponent<spacecraft>().rotating_planet.tag == "levelPlanet"
            && zoomPos == new Vector3(0,0,0)
            ) {

            levelPlanet = player.transform.GetChild(0).GetComponent<spacecraft>().rotating_planet;
            startZoomIn = true;
            zoomPos = levelPlanet.transform.position;
        }
        //trigger zoomOut after leaving the planet
        else if(player.transform.GetChild(0).GetComponent<spacecraft>().rotating_planet == null
            && zoomPos != new Vector3(0, 0, 0)) {
            endZoomIn = true;
         }
